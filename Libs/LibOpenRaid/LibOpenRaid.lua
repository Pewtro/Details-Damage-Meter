        ...
        if (LibAceSerializer) then
            ...
            if (okay) then
                ...
                if (okayUnitsInfo and unitsInfo) then
                    openRaidLib.UnitInfoManager.UnitData =
                        tempCache.copyCache(openRaidLib.UnitInfoManager.UnitData, unitsInfo)
                else
                    tempCache.AddDebugText("invalid UnitInfo")
                end

                if (okayCooldownsInfo and cooldownsInfo) then
                    openRaidLib.CooldownManager.UnitData =
                        tempCache.copyCache(openRaidLib.CooldownManager.UnitData, cooldownsInfo)
                else
...
function openRaidLib.commHandler.OnReceiveComm(
    self,
    event,
    prefix,
    text,
    ...
    if (prefix == CONST_COMM_PREFIX) then
        ...
        if (not dataTypePrefix) then
            openRaidLib.DiagnosticError(
                "Invalid dataTypePrefix from player:",
                sender,
                "data:",
                data,
                "dataTypePrefix:",
        ...
        if
            (dataTypePrefix ~= CONST_COMM_KEYSTONE_DATA_PREFIX and
                dataTypePrefix ~= CONST_COMM_KEYSTONE_DATAREQUEST_PREFIX)
         then
        ...
        if (not callbackTable) then
            openRaidLib.DiagnosticError(
                "Not callbackTable for dataTypePrefix:",
                dataTypePrefix,
                "from player:",
                sender,
                "data:",
...
local delayedTalentChange = function()
    openRaidLib.Schedules.NewUniqueTimer(
        0.5 + math.random(),
        talentChangedCallback,
        "TalentChangeEventGroup",
        "talentChangedCallback_Schedule"
    )
end
    ...
    ["UNIT_PET"] = function(unitId)
        if (UnitIsUnit(unitId, "player")) then
            openRaidLib.Schedules.NewUniqueTimer(
                1.1,
                function()
                    openRaidLib.internalCallback.TriggerEvent("playerPetChange")
                end,
                "mainControl",
                "petStatus_Schedule"
            )
...
function openRaidLib.mainControl.scheduleUpdatePlayerData()
    openRaidLib.Schedules.NewUniqueTimer(
        1.0,
        openRaidLib.mainControl.UpdatePlayerData,
        "mainControl",
        "updatePlayerData_Schedule"
    )
end
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_PLAYER_DEAD_PREFIX,
    function(data, unitName)
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_PLAYER_ALIVE_PREFIX,
    function(data, unitName)
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_FULLINFO_PREFIX,
    function(data, sourceName)
...
function openRaidLib.UnitInfoManager.SetUnitInfo(
    unitName,
    unitInfo,
    specId,
    renown,
    covenantId,
    talentsTableUnpacked,
    conduitsTableUnpacked,
    pvpTalentsTableUnpacked)
...
function openRaidLib.UnitInfoManager.AddUnitInfo(
    unitName,
    specId,
    renown,
    covenantId,
    ...
    openRaidLib.UnitInfoManager.SetUnitInfo(
        unitName,
        unitInfo,
        specId,
        renown,
    ...
    openRaidLib.publicCallback.TriggerCallback(
        "UnitInfoUpdate",
        openRaidLib.GetUnitID(unitName),
        openRaidLib.UnitInfoManager.UnitData[unitName],
        openRaidLib.UnitInfoManager.GetAllUnitsInfo()
    )
end
...
function openRaidLib.UnitInfoManager.OnReceiveUnitFullInfo(data, unitName)
    ...
    local borrowedTalentsTableUnpacked =
        openRaidLib.UnpackTable(data, borrowedTalentsTableIndex, false, false, borrowedTalentsSize)
    ...
    if (type(data[pvpTalentsTableIndex]) == "string" or not data[pvpTalentsTableIndex]) then
        ...
        openRaidLib.UnitInfoManager.AddUnitInfo(
            unitName,
            specId,
            playerInfo1,
            playerInfo2,
            talentsTableUnpacked,
            borrowedTalentsTableUnpacked,
            {0, 0, 0}
        )
        return
    end
...
function openRaidLib.UnitInfoManager.OnPlayerTalentChanged()
    ...
    openRaidLib.UnitInfoManager.SetUnitInfo(
        ...
        nil,
        nil,
        openRaidLib.UnitInfoManager.GetPlayerTalents()
    )

    --schedule send to the group
    openRaidLib.Schedules.NewUniqueTimer(
        1 + math.random(0, 1),
        openRaidLib.UnitInfoManager.SendTalentUpdate,
        "UnitInfoManager",
        "sendTalent_Schedule"
    )
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_PLAYERINFO_TALENTS_PREFIX,
    openRaidLib.UnitInfoManager.OnReceiveTalentsUpdate
)
...
function openRaidLib.UnitInfoManager.OnPlayerPvPTalentChanged()
    ...
    openRaidLib.UnitInfoManager.SetUnitInfo(
        ...
        nil,
        nil,
        openRaidLib.UnitInfoManager.GetPlayerPvPTalents()
    )

    --schedule send to the group
    openRaidLib.Schedules.NewUniqueTimer(
        1 + math.random(0, 1),
        openRaidLib.UnitInfoManager.SendPvPTalentUpdate,
        "UnitInfoManager",
        "sendPvPTalent_Schedule"
    )
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_PLAYERINFO_PVPTALENTS_PREFIX,
    openRaidLib.UnitInfoManager.OnReceivePvPTalentsUpdate
)
...
function openRaidLib.UnitInfoManager.OnLeaveCombat()
    openRaidLib.Schedules.NewUniqueTimer(
        1 + math.random(1, 4),
        openRaidLib.UnitInfoManager.SendPlayerInfoAfterCombat,
        "UnitInfoManager",
        "leaveCombat_Schedule"
    )
end
...
function openRaidLib.GearManager.OnLeaveCombat()
    openRaidLib.Schedules.NewUniqueTimer(
        1 + math.random(1, 4),
        openRaidLib.GearManager.SendGearInfoAfterCombat,
        "GearManager",
        "leaveCombat_Schedule"
    )
end
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_GEARINFO_DURABILITY_PREFIX,
    openRaidLib.GearManager.OnReceiveGearDurability
)
...
function openRaidLib.GearManager.UpdateUnitGearDurability(unitName, durability)
    ...
    if (unitGearInfo) then
        ...
        openRaidLib.publicCallback.TriggerCallback(
            ...
            durability,
            unitGearInfo,
            openRaidLib.GearManager.GetAllUnitsGear()
        )
    end
end
...
function openRaidLib.GearManager.AddUnitGearList(
    unitName,
    itemLevel,
    durability,
    weaponEnchant,
...
function openRaidLib.GearManager.OnReceiveGearFullInfo(data, unitName)
    ...
    local equippedGearListUnpacked =
        equippedGearListIndex and openRaidLib.UnpackTable(data, equippedGearListIndex, false, true, 4) or {}

    --add to the list of gear information
    openRaidLib.GearManager.AddUnitGearList(
        unitName,
        itemLevel,
        durability,
        weaponEnchant,
        noEnchantTableUnpacked,
        noGemsTableUnpacked,
        equippedGearListUnpacked
    )
end
...
local cooldownTimeLeftCheck_Ticker = function(tickerObject)
    ...
    local timeLeft, charges, startTimeOffset, duration, auraDuration =
        openRaidLib.CooldownManager.GetPlayerCooldownStatus(spellId)
    ...
    if (timeLeft == 0) then
        ...
        if (not openRaidLib.isNearlyEqual(tickerObject.cooldownTimeLeft, timeLeft, CONST_COOLDOWN_TIMELEFT_HAS_CHANGED)) then
            ...
            openRaidLib.CooldownManager.SendPlayerCooldownUpdate(
                spellId,
                timeLeft,
                charges,
                startTimeOffset,
    ...
    if (bUpdateLocally) then
        ...
        local timeLeft, charges, startTimeOffset, duration, auraDuration =
            openRaidLib.CooldownManager.GetPlayerCooldownStatus(spellId) --return 5 values
        --update the cooldown
        openRaidLib.CooldownManager.CooldownSpellUpdate(
            playerName,
            spellId,
            timeLeft,
            charges,
...
function openRaidLib.CooldownManager.CooldownSpellUpdate(
    unitName,
    spellId,
    newTimeLeft,
    newCharges,
...
function openRaidLib.GetCooldownStatusFromUnitSpellID(unitId, spellId)
    ...
    if (unitCooldownsTable) then
        local cooldownInfo = unitCooldownsTable[spellId]
        if (cooldownInfo) then
            timeLeft, charges, timeOffset, duration, updateTime, auraDuration =
                openRaidLib.CooldownManager.GetCooldownInfoValues(cooldownInfo)
        end
    end
...
function openRaidLib.CooldownManager.OnPlayerRess()
    ...
    if (openRaidLib.IsInGroup()) then
        openRaidLib.Schedules.NewUniqueTimer(
            ...
            openRaidLib.CooldownManager.SendAllPlayerCooldowns,
            "CooldownManager",
            "sendAllPlayerCooldowns_Schedule"
        )
    end
end
...
function openRaidLib.CooldownManager.OnEncounterEnd()
    ...
    openRaidLib.Schedules.NewUniqueTimer(
        0.1,
        openRaidLib.CooldownManager.CheckCooldownsAfterEncounterEnd,
        "CooldownManager",
        "encounterEndCooldownsCheck_Schedule"
    )
end
    ...
    if (#addedCooldowns > 0) then
        ...
        for spellId, cooldownInfo in pairs(cooldownsAddedUnpacked) do
            ...
            openRaidLib.publicCallback.TriggerCallback(
                ...
                spellId,
                cooldownInfo,
                openRaidLib.GetUnitCooldowns(unitName),
                openRaidLib.CooldownManager.UnitData
            )
        end
    end
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_COOLDOWNCHANGES_PREFIX,
    openRaidLib.CooldownManager.OnReceiveUnitCooldownChanges
)
    ...
    for spellId, cooldownInfo in pairs(newCooldownList) do
        if (not currentCooldowns[spellId]) then
            ...
            openRaidLib.publicCallback.TriggerCallback(
                ...
                spellId,
                cooldownInfo,
                openRaidLib.GetUnitCooldowns("player"),
                openRaidLib.CooldownManager.UnitData
            )
        end
    end
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_COOLDOWNUPDATE_PREFIX,
    function(data, unitName)
        --get data
...
function openRaidLib.CooldownManager.SendPlayerCooldownUpdate(
    spellId,
    cooldownTimeLeft,
    charges,
    startTimeOffset,
        ...
        CONST_COMM_COOLDOWNUPDATE_PREFIX ..
            "," ..
                spellId ..
                    "," ..
                        cooldownTimeLeft ..
                            "," .. charges .. "," .. startTimeOffset .. "," .. duration .. "," .. auraDuration
    openRaidLib.commHandler.SendCommData(dataToSend)
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_COOLDOWNFULLLIST_PREFIX,
    openRaidLib.CooldownManager.OnReceiveUnitCooldowns
)
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_COOLDOWNREQUEST_PREFIX,
    openRaidLib.CooldownManager.OnReceiveRequestForCooldownInfoUpdate
)
...
function openRaidLib.WipeKeystoneData()
    ...
    openRaidLib.publicCallback.TriggerCallback(
        ...
        unitName,
        keystoneInfo,
        openRaidLib.KeystoneInfoManager.KeystoneData
    )
    return true
end
...
local getKeystoneInfoToComm = function()
    ...
    local dataToSend =
        CONST_COMM_KEYSTONE_DATA_PREFIX ..
        "," ..
            keystoneInfo.level ..
                "," ..
                    keystoneInfo.mapID ..
                            ...
                            keystoneInfo.challengeMapID ..
                                "," ..
                                    keystoneInfo.classID ..
                                        "," .. keystoneInfo.rating .. "," .. keystoneInfo.mythicPlusMapID
    return dataToSend
end
...
function openRaidLib.KeystoneInfoManager.OnReceiveRequestData()
    ...
    if (IsInGroup() and not IsInRaid()) then
        openRaidLib.Schedules.NewUniqueTimer(
            ...
            "KeystoneInfoManager",
            "sendKeystoneInfoToParty_Schedule"
        )
    end

    if (IsInGuild()) then
        openRaidLib.Schedules.NewUniqueTimer(
            math.random(0, 3) + math.random(),
            openRaidLib.KeystoneInfoManager.SendPlayerKeystoneInfoToGuild,
            "KeystoneInfoManager",
            "sendKeystoneInfoToGuild_Schedule"
        )
    end
end
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_KEYSTONE_DATAREQUEST_PREFIX,
...
openRaidLib.commHandler.RegisterComm(
    CONST_COMM_KEYSTONE_DATA_PREFIX,
    openRaidLib.KeystoneInfoManager.OnReceiveKeystoneData
)
...
openRaidLib.internalCallback.RegisterCallback(
    "mythicDungeonEnd",
    openRaidLib.KeystoneInfoManager.OnMythicDungeonFinished
)
...
C_Timer.After(
    ...
    function()
        ...
        vintageCDTrackerFrame:SetScript(
            "OnEvent",
            function(self, event, ...)
